...
export function loadVideoWithPlyr(videoTarget, targetUrl, resumeTime = 0, options = {}) {
  ...
  console.log(`[ShowVerse Player] loadVideoWithPlyr called! Target: ${targetUrl}`);
  ...
  if (!videoElement) {
    console.error("[ShowVerse Player] Target video element not found in DOM!", videoTarget);
    return null;
  }
  ...
  if (!streamUrl) {
    console.error("[ShowVerse Player] Provided stream URL is empty!");
    hideVideoSpinner(videoElement);
  ...
  const handleVideoFailure = (event) => {
    ...
    console.error(`[ShowVerse Player] Video failure: "${type}" on "${streamUrl}"`, err);
